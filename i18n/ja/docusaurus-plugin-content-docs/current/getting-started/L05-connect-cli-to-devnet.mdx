...
このフレーズを[Slushウォレット](./L01-install-slush.mdx)にインポートすると、CLIと同じアドレスをブラウザ拡張でも使えます（オプション）。
:::

<details>
<summary>Slushにインポートする方法（オプション）</summary>

1. ブラウザ拡張機能のSlush Walletを開きます（要求されればパスワード入力）
2. 右下の **ユーザーアイコン** → **Manage Account** → **Add account** を開きます
3. **Import passphrase** を選択します
4. CLIのリカバリーフレーズを入力します

インポートが完了すると、SlushでもCLIと同じアドレスが使えるようになります。

:::tip
必須ではありませんが、CLIで作成したアドレスをSlushにインポートしておくと、
残高やアドレスを確認しやすくなります。
:::
