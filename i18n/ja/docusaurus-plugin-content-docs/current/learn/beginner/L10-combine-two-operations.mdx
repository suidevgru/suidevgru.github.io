...
<!-- ![Editor画面。ここでノードを追加・接続してPTBを構築します](./img/L10_editor_initial.png) -->
...
<!-- ![右クリックメニュー。Sui Commands → SplitCoins を選択](./img/L10_right_click_menu.png) -->
...
:::note なぜ SplitCoins が必要？
Suiでは、送金は「コインオブジェクト単位」で行われます。
そのため、送金額ぴったりのコインを SplitCoins で作り、
そのコインを TransferObjects で送ります。
:::
...
<!-- ![1つ目の送金設定が完了した状態](./img/L10_first_transfer_complete.png) -->
...
:::note gas コインについて
PTB Builderの `gas` は「ガス代専用」ではなく、
通常の SUI コインとしても使われます。

そのため、1つの gas コインを複数回 SplitCoins して
送金に使うことができます。
:::
...
<!-- ![2つの送金を合成した完成形フロー](./img/L10_complete_flow.png) -->
...
<!-- ![トランザクション実行成功画面](./img/L10_execution_success.png) -->
...
:::tip
エクスポートされるコードは、CLI で書く PTB と本質的に同じ構造です。
:::
