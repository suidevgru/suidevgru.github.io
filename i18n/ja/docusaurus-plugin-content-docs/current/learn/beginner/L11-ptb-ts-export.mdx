...
このレッスンでは、PTB Builderで作成したPTBを**TypeScriptコードとしてエクスポート**する方法を学びます。**難しくありません**。ボタンをクリックするだけです。
...
- **GUIで組み立てた内容をコードで確認**できる
- **学習の足がかり**になる（コードの書き方がわからなくても、GUIで組み立ててエクスポートすれば参考になる）
- **実装のスタート地点**として活用できる
...
- [L10: 2操作を合成](/docs/learn/beginner/L10-combine-two-operations) を完了している（推奨）
...
エクスポートしたコードは、Unit 4（TypeScript SDK）で実際に使います。GUIで組み立てた内容がコードでどう表現されるか、この段階で確認しておくと、SDKを学ぶときの理解が深まります。
